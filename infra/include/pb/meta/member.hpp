...
namespace pb::infra::meta
{
	...
	namespace member_detail
	{
		...
		constexpr auto invoke(const Function& function, Args&&... args) noexcept -> decltype(auto)
		{
			...
			else { PB_SEMANTIC_STATIC_UNREACHABLE(); }
		}
	}
	...
	namespace member_detail
	{
		...
		[[nodiscard]] constexpr auto member_index() noexcept -> std::size_t
		{
			return []<std::size_t... Index>(std::index_sequence<Index...>) noexcept -> std::size_t
			{
				...
				PB_COMPILER_DISABLE_CLANG_WARNING(-Wunused-value)
		...
		[[nodiscard]] constexpr auto member_of_name(T&& object) noexcept -> decltype(auto)
		{
			...
			else
			{
				...
				if constexpr (sub_ref_index == std::string_view::npos and sub_ptr_index == std::string_view::npos)
				{
					...
					{
						PB_SEMANTIC_STATIC_UNREACHABLE("If you see this message, you've passed in the wrong member name.");
					}
					else
					{
...
#include <pb/meta/member.visit.inl>
