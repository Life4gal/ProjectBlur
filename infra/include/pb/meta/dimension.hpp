...
namespace pb::infra::meta
{
	...
	namespace dimension_detail
	{
		...
		struct dimension_walker
		{
			...
			PB_COMPILER_DISABLE_WARNING(4244)
#endif
	...
	private:
		[[nodiscard]] constexpr auto rep() noexcept -> dimension_type&
		{
			return *static_cast<dimension_type*>(this);
		}

		[[nodiscard]] constexpr auto rep() const noexcept -> const dimension_type&
		{
			return *static_cast<const dimension_type*>(this);
		}
...
namespace std
{
	...
	struct tuple_element<Index, pb::infra::meta::dimension_detail::dimension_wrapper<T, N>> // NOLINT(cert-dcl58-cpp)
	{
		using type = T;
	};

	template<typename T, std::size_t N>
	struct tuple_size<pb::infra::meta::dimension_detail::dimension_wrapper<T, N>> : std::integral_constant<std::size_t, N> {}; // NOLINT(cert-dcl58-cpp)
}

#include <pb/meta/dimension.cache.inl>
