...
#include <pb/meta/name.hpp>

// ReSharper disable once CppInconsistentNaming
enum class _PbMetaEnumeration_DO_NOT_USE : std::uint8_t
{
	_
};
...
namespace pb::infra::meta
{
	namespace enumeration_detail
	{
		...
		[[nodiscard]] constexpr auto name_of() noexcept -> std::string_view
		{
			...
			// MSVC
			// class std::basic_string_view<char,struct std::char_traits<char> > `__calling_convention` `namespace`::get_full_function_name<`_PbMetaEnumeration_DO_NOT_USE::_`>(void) noexcept
			constexpr std::string_view full_function_name = meta::get_full_function_name<_PbMetaEnumeration_DO_NOT_USE::_>();
			constexpr auto full_function_name_size = full_function_name.size();

			constexpr std::string_view dummy_enum_value_name = "_PbMetaEnumeration_DO_NOT_USE::_";
...
#elif defined(PB_COMPILER_CLANG) or defined(PB_COMPILER_CLANG_CL)
			// CLANG/CLANG-CL
			// std::string_view `namespace`::get_full_function_name() [V = `_PbMetaEnumeration_DO_NOT_USE::_`]
			constexpr std::string_view full_function_name = meta::get_full_function_name<_PbMetaEnumeration_DO_NOT_USE::_>();
			constexpr auto full_function_name_size = full_function_name.size();

			constexpr std::string_view dummy_struct_name{"_PbMetaEnumeration_DO_NOT_USE::_"};
...
#else
			// GCC
			// constexpr std::string_view `namespace`::get_full_function_name() [with auto <anonymous> = `_PbMetaEnumeration_DO_NOT_USE::_`; std::string_view = std::basic_string_view<char>]
			constexpr std::string_view full_function_name = meta::get_full_function_name<_PbMetaEnumeration_DO_NOT_USE::_>();
			constexpr auto full_function_name_size = full_function_name.size();
			...
			PB_COMPILER_DISABLE_WARNING_PUSH
					PB_COMPILER_DISABLE_WARNING(-Wenum -constexpr-conversion)
#endif

			auto full_name = meta::get_full_function_name<EnumValue>();

#if defined(PB_COMPILER_APPLE_CLANG) or defined(PB_COMPILER_CLANG_CL) or defined(PB_COMPILER_CLANG)
			PB_COMPILER_DISABLE_WARNING_POP
#endif
...
#error "fixme"
		...
		template<typename EnumType, bool ExcludeCombination>
			requires std::is_enum_v<EnumType>
		[[nodiscard]] constexpr auto flag_dynamic_enum_values() noexcept -> std::vector<EnumType>
		{
			...
			if constexpr (ExcludeCombination)
			{
				constexpr std::size_t this_count = 1;

				const auto check = [&values]<std::size_t ThisIndex>() noexcept -> void
				{
					if constexpr (constexpr auto bits = flag_get_bits_combination<EnumType, this_count, ThisIndex>();
		...
		template<typename EnumType, bool ExcludeCombination>
			requires std::is_enum_v<EnumType>
		struct cached_flag_dynamic_enum_values_size : std::integral_constant<std::size_t, flag_dynamic_enum_values<EnumType, ExcludeCombination>().size()> {};
		...
		[[nodiscard]] constexpr auto is_flag_with_user_defined() noexcept -> bool
		{
			...
			else
			{
				...
				constexpr auto size = cached_flag_dynamic_enum_values_size<EnumType, true>::value;
				constexpr auto required = static_cast<cached_flag_dynamic_enum_values_size<EnumType, true>::value_type>(6);
			...
			else
			{
				...
				else
				{
					...
					)
					{
						if constexpr (StrideShift == 0)
						{
							PB_SEMANTIC_STATIC_UNREACHABLE("test `meta::enumeration_detail::name_of` and check `is_valid_enum`");
						}
						else
						{
		...
		[[nodiscard]] constexpr auto trim_full_name(const std::string_view name) noexcept -> std::string_view
		{
			...
			else if constexpr (Policy == EnumNamePolicy::VALUE_ONLY)
			{
				...
				if constexpr (std::is_scoped_enum_v<EnumType>)
				{
					PB_ERROR_ASSUME(last_double_colon != std::string_view::npos);
		...
		{
			constexpr auto size = cached_flag_dynamic_enum_values_size<EnumType, false>::value;
				...
				return //
						return_type{
								...
								typename return_type::value_type{
										//
										flag_dynamic_enum_values<EnumType, false>()[Index],
										trim_full_name<EnumType, Policy>(
											name_of<static_cast<EnumType>(flag_dynamic_enum_values<EnumType, false>()[Index])>()
										)
								}...
						};
	...
	[[nodiscard]] constexpr auto name_of(const EnumType enum_value) noexcept -> std::string_view
	{
		...
		if (const auto it = std::ranges::find(list,
		                                      enum_value,
