...
namespace pb::infra::meta
{
	...
	using basic_string_view = std::basic_string_view<ValueType>;
	...
	namespace string_detail
	{
		...
		template<typename Container, typename SizeType = typename Container::size_type>
			requires requires(const Container& container, const SizeType index) { container[index]; }
		struct default_getter
		{
			...
			[[nodiscard]] constexpr auto operator()(const Container& container, const SizeType index) const noexcept(noexcept(container[index])) -> decltype(auto)
			{
				return container[index];
			}
		};
		...
		enum class MetaStringDerivedCategory : std::uint8_t
		{
			MEMBER,
			MEMBER_FUNCTION,
			STATIC,
		...
		template<typename T>
			requires(
				        std::is_member_object_pointer_v<decltype(&T::value)> and
				        not std::is_member_function_pointer_v<decltype(&T::value)>
			        ) and
		...
		template<typename T>
			requires(
				        not std::is_member_object_pointer_v<decltype(&T::value)> and
				        std::is_member_function_pointer_v<decltype(&T::value)>
			        ) and requires(const T& derived)
			        {
				        {
					        derived.data()
		...
		template<typename T>
			requires(
				        not std::is_member_object_pointer_v<decltype(&T::value)> and
				        not std::is_member_function_pointer_v<decltype(&T::value)>
			        ) and (not std::is_invocable_v<decltype(T::value)>)
		struct pointer_traits<T>
		{
			// const pointer
		...
		template<typename T>
			requires(
				        not std::is_member_object_pointer_v<decltype(&T::value)> and
				        not std::is_member_function_pointer_v<decltype(&T::value)>
			        ) and (std::is_invocable_v<decltype(T::value)>)
		struct pointer_traits<T>
		{
			// const pointer
		...
		private:
				...
				requires(not lazy_is_static<derived_type>())
			{
				...
				{
					PB_SEMANTIC_STATIC_UNREACHABLE();
				}
			}
			...
			template<typename StringType>
			constexpr static auto is_constructible_from_view_v = std::is_constructible_v<StringType, basic_string_view<value_type>>;
		...
		public:
			...
			[[nodiscard]] constexpr auto as() const noexcept -> StringType //
				requires(not lazy_is_static<derived_type>())
			{
				return StringType{this->operator basic_string_view<value_type>()};
			}
			...
			[[nodiscard]] constexpr static auto as() noexcept -> StringType //
				requires(lazy_is_static<derived_type>())
			{
				return StringType{meta_string_base::rep_value(), meta_string_base::rep_size()};
			}
			...
			{
				return lhs.template as<basic_string_view<value_type>>() == rhs.template as<basic_string_view<value_type>>();
			}
			...
			{
				return this->template as<basic_string_view<value_type>>() == rhs.template as<basic_string_view<value_type>>();
			}
			...
			{
				return meta_string_base::as<basic_string_view<value_type>>() == rhs.template as<basic_string_view<value_type>>();
			}

			template<typename String>
				requires(std::is_constructible_v<basic_string_view<value_type>, String> and not is_meta_string_base_v<String>)
			[[nodiscard]] friend constexpr auto operator==(const derived_type& lhs, const String& rhs) noexcept(std::is_nothrow_constructible_v<basic_string_view<value_type>, String>) -> bool //
			{
				//
				return lhs.template as<basic_string_view<value_type>>() == basic_string_view<value_type>{rhs};
			}

			template<typename String>
				requires(std::is_constructible_v<basic_string_view<value_type>, String> and not is_meta_string_base_v<String>)
			[[nodiscard]] constexpr auto match(const String& rhs) const noexcept(std::is_nothrow_constructible_v<basic_string_view<value_type>, String>) -> bool //
				requires(not lazy_is_static<derived_type>())
			{
				//
				return *this == rhs;
			...
			template<typename String>
				requires(std::is_constructible_v<basic_string_view<value_type>, String> and not is_meta_string_base_v<String>)
			[[nodiscard]] constexpr static auto match(const String& rhs) noexcept(std::is_nothrow_constructible_v<basic_string_view<value_type>, String>) -> bool //
				requires(lazy_is_static<derived_type>())
			{
				//
				return meta_string_base::as<basic_string_view<value_type>>() == basic_string_view<value_type>{rhs};
			}
			...
			match(
				...
				requires(not lazy_is_static<derived_type>())
			{
				if (std::ranges::equal(std::ranges::size(container), rep_size()))
				{
					return false;
				}
		...
		[[nodiscard]] consteval auto contains_zero() noexcept -> bool
		{
			...
			{
				return contains_zero<T, Cs...>();
			}
		}
	} // namespace string_detail

	template<typename S>
	concept basic_char_array_t = string_detail::is_basic_char_array_v<S>;

	template<typename S>
	concept basic_fixed_string_t = string_detail::is_basic_fixed_string_v<S>;
	...
	struct basic_char_array : string_detail::meta_string_base<basic_char_array<T, Cs...>, T, std::size_t>
	{
		...
		[[nodiscard]] constexpr static const_pointer begin() noexcept
		{
			return std::ranges::begin(value);
		}

		[[nodiscard]] constexpr static const_pointer end() noexcept
		{
		...
		{
			return basic_fixed_string<value_type, N>{basic_string_view<value_type>{value, N}};
		}
		...
		// template<typename String>
		// 	requires(not basic_char_array_t<String>) and std::is_constructible_v<basic_string_view<value_type>, String>
		// [[nodiscard]] constexpr auto operator<=>(const String& rhs) const noexcept(std::is_nothrow_constructible_v<basic_string_view<value_type>, String>) -> auto
		// {
		// 	//
		// 	return this->template as<basic_string_view<value_type>>() <=> basic_string_view<value_type>{rhs};
		// }
		//
		// // string <=> basic_char_array
		// template<typename String>
		// 	requires(not basic_char_array_t<String>) and std::is_constructible_v<basic_string_view<value_type>, String>
		// [[nodiscard]] friend constexpr auto operator<=>(const String& lhs, const basic_char_array& rhs) noexcept(std::is_nothrow_constructible_v<basic_string_view<value_type>, String>) -> auto
		// {
		// 	//
		// 	return basic_string_view<value_type>{lhs} <=> rhs.template as<basic_string_view<value_type>>();
		// }
	};
	...
	struct basic_fixed_string : string_detail::meta_string_base<basic_fixed_string<T, N>, T, std::size_t>
	{
		...
		[[nodiscard]] constexpr auto begin() noexcept -> pointer
		{
			return value;
		}

		[[nodiscard]] constexpr auto begin() const noexcept -> const_pointer
		{
			return value;
		}
		...
		[[nodiscard]] constexpr auto operator[](const size_type index) const noexcept -> value_type
		{
			return value[index];
		}
		...
		// template<typename String>
		// 	requires(not basic_fixed_string_t<String>) and std::is_constructible_v<basic_string_view<value_type>, String>
		// [[nodiscard]] constexpr auto operator<=>(const String& rhs) const noexcept(std::is_nothrow_constructible_v<basic_string_view<value_type>, String>) -> auto
		// {
		// 	//
		// 	return this->template as<basic_string_view<value_type>>() <=> basic_string_view<value_type>{rhs};
		...
		// template<typename String>
		// 	requires(not basic_fixed_string_t<String>) and std::is_constructible_v<basic_string_view<value_type>, String>
		// [[nodiscard]] friend constexpr auto operator<=>(const String& lhs, const basic_fixed_string& rhs) noexcept(std::is_nothrow_constructible_v<basic_string_view<value_type>, String>) -> auto
		// {
		// 	//
		// 	return basic_string_view<value_type>{lhs} <=> rhs.template as<basic_string_view<value_type>>();
		// }
...
namespace std
{
	...
	template<pb::infra::meta::basic_char_array_t FixedString, typename String, template<typename> typename Q1, template<typename> typename Q2>
		requires std::is_constructible_v<pb::infra::meta::basic_string_view<typename FixedString::value_type>, String>
	struct basic_common_reference<FixedString, String, Q1, Q2> // NOLINT(cert-dcl58-cpp)
	{
		using type = pb::infra::meta::basic_string_view<typename FixedString::value_type>;
	};

	template<typename String, pb::infra::meta::basic_char_array_t FixedString, template<typename> typename Q1, template<typename> typename Q2>
		requires std::is_constructible_v<pb::infra::meta::basic_string_view<typename FixedString::value_type>, String>
	...
	// for `std::totally_ordered_with`
	template<pb::infra::meta::basic_fixed_string_t FixedString, typename String, template<typename> typename Q1, template<typename> typename Q2>
		requires std::is_constructible_v<pb::infra::meta::basic_string_view<typename FixedString::value_type>, String>
	struct basic_common_reference<FixedString, String, Q1, Q2> // NOLINT(cert-dcl58-cpp)
	{
		using type = pb::infra::meta::basic_string_view<typename FixedString::value_type>;
	...
	template<typename String, pb::infra::meta::basic_fixed_string_t FixedString, template<typename> typename Q1, template<typename> typename Q2>
		requires std::is_constructible_v<pb::infra::meta::basic_string_view<typename FixedString::value_type>, String>
	struct basic_common_reference<String, FixedString, Q1, Q2> // NOLINT(cert-dcl58-cpp)
	{
		using type = pb::infra::meta::basic_string_view<typename FixedString::value_type>;
