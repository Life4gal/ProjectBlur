...
namespace pb::infra::graphics
{
	...
	class [[nodiscard]] alignas(std::alignment_of<universal_32_bit_color_type>) Color final
	{
	public:
		...
		[[nodiscard]] constexpr static auto from(const universal_32_bit_color_type color, const color_detail::color_format_type<Format> = {}) noexcept -> Color
		{
			...
			{
				return {.red = to_value(color >> 16), .green = to_value(color >> 8), .blue = to_value(color >> 0), .alpha = to_value(0xff)};
			}
			else if constexpr (Format == ColorFormat::R_G_B_A)
			{
				return {.red = to_value(color >> 24), .green = to_value(color >> 16), .blue = to_value(color >> 8), .alpha = to_value(color >> 0)};
			}
			else if constexpr (Format == ColorFormat::A_R_G_B)
			{
				return {.red = to_value(color >> 16), .green = to_value(color >> 8), .blue = to_value(color >> 0), .alpha = to_value(color >> 24)};
			}
			else if constexpr (Format == ColorFormat::B_G_R)
			{
				return {.red = to_value(color >> 0), .green = to_value(color >> 8), .blue = to_value(color >> 16), .alpha = to_value(0xff)};
			}
			else if constexpr (Format == ColorFormat::B_G_R_A)
			{
				return {.red = to_value(color >> 8), .green = to_value(color >> 16), .blue = to_value(color >> 24), .alpha = to_value(color >> 0)};
			}
			else if constexpr (Format == ColorFormat::A_B_G_R)
			{
				return {.red = to_value(color >> 0), .green = to_value(color >> 8), .blue = to_value(color >> 16), .alpha = to_value(color >> 24)};
			}
			else
			{
				std::unreachable();
		...
		[[nodiscard]] constexpr auto to(const color_detail::color_format_type<Format> = {}) const noexcept -> universal_32_bit_color_type
		{
			...
			if constexpr (Format == ColorFormat::R_G_B)
			{
				return to_value(red << 16) | to_value(green << 8) | to_value(blue << 0);
			}
			else if constexpr (Format == ColorFormat::R_G_B_A)
			{
				return to_value(red << 24) | to_value(green << 16) | to_value(blue << 8) | to_value(alpha << 0);
			}
			else if constexpr (Format == ColorFormat::A_R_G_B)
			{
				return to_value(red << 16) | to_value(green << 8) | to_value(blue << 0) | to_value(alpha << 24);
			}
			else if constexpr (Format == ColorFormat::B_G_R)
			{
				return to_value(red << 0) | to_value(green << 8) | to_value(blue << 16);
			}
			else if constexpr (Format == ColorFormat::B_G_R_A)
			{
				return to_value(red << 8) | to_value(green << 16) | to_value(blue << 24) | to_value(alpha << 0);
			}
			else if constexpr (Format == ColorFormat::A_B_G_R)
			{
				return to_value(red << 0) | to_value(green << 8) | to_value(blue << 16) | to_value(alpha << 24);
			}
			else
			{
				std::unreachable();
		...
		[[nodiscard]] constexpr auto transparent() const noexcept -> Color
		{
			return {.red = red, .green = green, .blue = blue, .alpha = 0};
		}

		[[nodiscard]] constexpr auto luminance() const noexcept -> float
		{
			return (0.299f * red + 0.587f * green + 0.114f * blue) / 255.0f;
		}

		[[nodiscard]] constexpr auto blend(const Color& other, const float t) const noexcept -> Color
		{
			// ReSharper disable once IdentifierTypo
			const auto lerp = [t](const value_type a, const value_type b) noexcept -> value_type
			{
				return static_cast<value_type>(a * (1 - t) + b * t);
			};

			return {
					lerp(red, other.red),
					lerp(green, other.green),
					lerp(blue, other.blue),
					lerp(alpha, other.alpha)
			};
		}
	...
	static_assert(sizeof(Color) == sizeof(universal_32_bit_color_type));

	[[nodiscard]] constexpr auto operator""_rgb(const universal_32_bit_color_type value) noexcept -> Color
	{
		return Color::from<ColorFormat::R_G_B>(value);
	}

	[[nodiscard]] constexpr auto operator""_rgba(const universal_32_bit_color_type value) noexcept -> Color
	{
		return Color::from<ColorFormat::R_G_B_A>(value);
	}

	[[nodiscard]] constexpr auto operator""_argb(const universal_32_bit_color_type value) noexcept -> Color
	{
		return Color::from<ColorFormat::A_R_G_B>(value);
	}

	[[nodiscard]] constexpr auto operator""_bgr(const universal_32_bit_color_type value) noexcept -> Color
	{
		return Color::from<ColorFormat::B_G_R>(value);
	}

	[[nodiscard]] constexpr auto operator""_bgra(const universal_32_bit_color_type value) noexcept -> Color
	{
		return Color::from<ColorFormat::B_G_R_A>(value);
	}

	[[nodiscard]] constexpr auto operator""_abgr(const universal_32_bit_color_type value) noexcept -> Color
	{
		return Color::from<ColorFormat::A_B_G_R>(value);
	}

	namespace colors
	{
		using color_type = Color;
		using value_type = Color::value_type;

		constexpr auto build_color = [](const value_type red, const value_type green, const value_type blue) noexcept -> Color
		{
...
#endif
			tuple_element<Index, pb::infra::graphics::Color> // NOLINT(cert-dcl58-cpp)
	{
		using type = pb::infra::graphics::Color::value_type;
	};

	template<>
	struct tuple_size<pb::infra::graphics::Color> // NOLINT(cert-dcl58-cpp)
			: std::integral_constant<std::size_t, 4> {};

	template<>
	struct formatter<pb::infra::graphics::Color> // NOLINT(cert-dcl58-cpp)
	{
		...
		constexpr auto parse(ParseContext& context) noexcept -> auto
		{
			auto it = context.begin();
			if (const auto end = context.end();
				it != end)
			{
				switch (*it)
				{
					...
					case 'a':
					{
						type = FormatType::RGBA;
						break;
					}
					case 'c':
					{
						type = FormatType::CSS_RGB;
						break;
					}
					case 'C':
					{
						type = FormatType::CSS_RGBA;
						break;
					}
					default:
					{
		...
		auto format(const pb::infra::graphics::Color& color, FormatContext& context) const noexcept -> auto
		{
			switch (type)
			{
				case FormatType::HEX:
				{
					return std::format_to(context.out(), "#{:08X}", color.to<pb::infra::graphics::ColorFormat::A_R_G_B>());
				}
				case FormatType::RGB:
				{
				...
				{
					return std::format_to(context.out(), "rgba({}, {}, {}, {:.2f})", color.red, color.green, color.blue, color.alpha / 255.0f);
				}
				case FormatType::CSS_RGB:
				{
					return std::format_to(context.out(), "rgb({} {} {})", color.red, color.green, color.blue);
				}
				case FormatType::CSS_RGBA:
				{
					return std::format_to(context.out(), "rgba({} {} {} / {:.2f})", color.red, color.green, color.blue, color.alpha / 255.0f);
				}
			}
